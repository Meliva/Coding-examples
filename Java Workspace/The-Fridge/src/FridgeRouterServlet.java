...
public class FridgeRouterServlet extends HttpServlet
{
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		//Object responseObject = CONTENT_TYPE + CHARACTER_ENCODING;
		...
		try 
		{
			...
			modelName = modelName.substring(1).toLowerCase();
			FirstLetter = FirstLetter.toUpperCase();
			modelName = FirstLetter+modelName;
			...
			//Object controllerInstance = constructor;	
		...
		catch (Exception exp)
		{
			...
			if (exp instanceof InvocationTargetException) 
			{
				...
				{
					response.setStatus(HttpServletResponse.SC_METHOD_NOT_ALLOWED);
					message = "This update is not allowed";
				}

				if (exp instanceof ValidationException) 
				{
					response.setStatus(HttpServletResponse.SC_PRECONDITION_FAILED);
					message = "This validation has failed";
				}
			}
